#!/usr/bin/env python3
"""Train Collatz embeddings v5: hard negative mining from raw metrics space.

Key changes from v4:
- Mine hard negatives from raw normalized metrics (32-dim), not from model embeddings.
  Raw metrics already have the 84% lift signal, so negatives are meaningful.
- Family-positive-only training (no self-noise pairs).
- Metrics-aware auxiliary loss preserves trajectory structure.
- V1 single-encoder architecture.
- Evaluate every 10 epochs.

The core insight from v4: the model's own embeddings start bad, so they mine
bad negatives, which the model then optimizes away. Mining from raw metrics
space bypasses that feedback loop entirely.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Collatz embeddings v5: raw-metrics hard negatives."
    )
    ...
    parser.add_argument("--output-dir", default="/home/ryancox/3xN1/data/generated/contrastive_v5")
    ...
    parser.add_argument("--use-v1-encoder", action="store_true", default=True,
        help="Use single-branch encoder (v1 architecture)")
    ...
    parser.add_argument("--hard-negatives", type=int, default=20,
        help="Number of hard negatives mined from raw metrics space")
...
def read_families(path, starts, primary_label):
    ...
    with path.open(newline="") as f:
        ...
        for row in reader:
            ...
            labels[idx] = label
            counts[label] += 1
    print(f"  Labels: {len(counts)} unique, top 5: {counts.most_common(5)}")
    return labels, dict(counts)


def mine_hard_negatives_raw_metrics(metrics_tensor, labels, k=20, chunk_size=10000):
    """Mine hard negatives from raw normalized metrics space.

    This avoids the v4 problem: the model's own embeddings are bad early
    in training, so mining from them creates a feedback loop of bad negatives.
    Raw metrics already carry the 84% signal, so mining from metrics space
    produces meaningful negatives.
...
def info_nce_with_hard_negatives(z_a, z_b, hard_neg_indices, raw_negatives=None, temperature=0.07):
    """InfoNCE with hard negatives from raw metrics space.

    When mining from raw metrics, hard_neg_indices points to rows in the
    full dataset. We compute their embeddings via raw_negatives.
    """
    ...
    if hard_neg_indices is not None and hard_neg_indices.shape[1] > 0:
        if raw_negatives is not None:
            # raw_negatives: precomputed embeddings for all items
...
def main():
    ...
    # Load positive pairs (family pairs only)
    ...
    for epoch in range(args.epochs):
        ...
        for bs in range(0, len(starts), args.batch_size):
            ...
            # Use raw-metrics hard negatives (fixed from v4)
            ...
            if args.pair_mode in ("family_pairs", "combined") and len(pos_a) > 0:
                try:
                    ...
                    loss_family = info_nce_with_hard_negatives(z_a2, z_b2, hard_neg, all_embeddings, args.temperature)
            ...
            if loss_family is not None:
                loss = (1.0 - args.metrics_loss_weight) * loss_family + args.metrics_loss_weight * loss_metrics
            else:
                loss = loss_metrics
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(f"  epoch {epoch+1}/{args.epochs} loss={epoch_loss:.4f} lift={lift_eval:.4f} best_lift={best_lift:.4f} lr={new_lr:.6f}", flush=True)
    ...
    final_metrics = {
        "dataset_type": "collatz_contrastive_embeddings_v5",
        "tool": "research/contrastive_train_v5.py",
        "status": "complete",
        "device": str(device),
    ...
    print(f"\ncontrastive_v5 feature_set={args.feature_set}", flush=True)
    print(f"  pair_mode={args.pair_mode} rows={len(starts)} lift={purity_lift:.4f} best_lift={best_lift:.4f}", flush=True)
    print(f"  hard_negative_source=raw_metrics hard_negatives={args.hard_negatives}", flush=True)
    print(f"  metrics_loss_weight={args.metrics_loss_weight}", flush=True)
