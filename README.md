...
- Audit: `670,000,000` rows over `1..670,000,000`; full audit completed: `true`.
- Coverage: topology `113,889` rows (`0.017%` of audit); stratified evidence sample `113,889` rows (`0.017%`).
- Neural result: `100,000` sample rows; GPU used: `true`; parallel jobs completed: `6`.
- Learned lift: weakest range `8.607%`, fold minimum `8.863%`, numeric-adjacency lift `5.204%`.
- Best current ablation: `metrics-only at 12.564%`.
