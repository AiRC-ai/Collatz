...
This document preserves the complete experimental history, evidence snapshots,
and operating guide for the project. Start with the
[project overview](../README.md) for the supported build path and current
findings.
...
![Current Collatz evidence dashboard snapshot](media/dashboard-summary.svg)

![Historical Collatz evidence trend](media/evidence-history.svg)
...
![Original (metrics-only) vs ours (hybrid)](media/v9-v10-supervised-chart.svg)
...
[NEURAL_ENGINE_V9.md](NEURAL_ENGINE_V9.md).
...
![v11 fine-structure: prototypical few-shot + re-clustered families](media/v11-fine-structure-chart.svg)

Source: `data/generated/contrastive_v11/metrics.json`. Full write-up:
[NEURAL_ENGINE_V11.md](NEURAL_ENGINE_V11.md).
...
[NEURAL_ENGINE_V12.md](NEURAL_ENGINE_V12.md).
...
[NEURAL_ENGINE_V13.md](NEURAL_ENGINE_V13.md).
...
write-up: [PROOF_WORK.md](PROOF_WORK.md).
...
- Meaning: Held-out evaluation supports direct supervised structure learning: family-disjoint prototypical retrieval reaches +82.3% lift versus +25.1% for raw metrics on unseen families, and the coarse multi-task embedding beats raw metrics on all three held-out labels. The v13 cluster-proxy control underperforms raw metrics (+21.0% versus +24.7%), so re-clustering remains a diagnostic rather than a promoted training result. This is empirical evidence, not a proof.
...
- Interpretation: `held-out supervised structure with negative controls`; The v12 family-disjoint evaluation trains on 80% of eligible families and retrieves only against the held-out 20%: direct prototypical learning reaches +82.3% mean lift across three seeds versus +25.1% for raw metrics on the same pool. Row-disjoint coarse evaluation also beats raw metrics for range_band, bit_length, and peak_ratio_bucket. In v13, a cluster-proxy representation fails to beat raw metrics on the real fine-family target (+21.0% versus +24.7%), so that proxy is recorded as a negative control rather than a win.
...
- Promotion blockers: `metrics_only_exceeds_hybrid, matched_controls_incomplete, missing_lift_statistics`.
- Source alignment: `5,186 / 5,191` matched; unknown unmatched rows `0`.
- Next experiment: Evaluate non-circular feature hypotheses for direct fine-family retrieval under family-disjoint splits, using raw metrics and the failed cluster-proxy representation as mandatory controls. Separately, test richer invariant classes with the existing bounded falsifier before considering formalization.
...
![v3 Ablation Chart](media/v3-results-chart.svg)

![Legacy Baseline Chart](media/legacy-results-chart.svg)

![v4 Ablation Chart](media/v4-ablation-chart.svg)

![v5 Ablation Chart](media/v5-ablation-chart.svg)

![Loss Curves](media/loss-curves.svg)
...
![Real Collatz path-image tensor atlas generated from n=27](media/path-image-atlas.svg)
...
Regenerate the public path-image atlas from the real encoder:
...
Regenerate the dashboard image from the canonical public evidence JSON:
...
same canonical evidence file as the generated snapshot, and it intentionally omits
...
from the source-covered safe-metric export so evidence alignment, public claims,
...
5. Publishes the canonical evidence summary used by the research record and dashboard claims.
